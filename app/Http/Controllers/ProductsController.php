...
class ProductsController extends Controller
{
    ...
    public function products($url=null){
        ...
        if(!empty($_GET['pricefrom']) && !empty($_GET['priceto'])){
            $productsAll = $productsAll->whereBetween('products.price',[$_GET['pricefrom'], $_GET['priceto']]);
        }
    ...
    public function filter(Request $request){
        ...
        $pricefromUrl="";
        $pricetoUrl="";
        if(!empty($data['pricefrom']) && !empty($data['priceto'])){
            $pricefromUrl = "&pricefrom=".$data['pricefrom'];
            $pricetoUrl = "&priceto=".$data['priceto'];
        }

        $finalUrl = "products/".$data['url']."?".$colorUrl.$sleeveUrl.$patternUrl.$sizeUrl.$pricefromUrl.$pricetoUrl;
    ...
    public function addtowishlist(Request $request)
    {
        if(empty(Auth::user()->email)){
            return response()->json(['status' => 'err', 'message' => 'Harap Login Terlebih Dahulu.']);
        }else{
            $data['user_email'] = Auth::user()->email;
            $insert = DB::table('wishlist')
                        ->insert(['product_id' => $request->get('productid'), 'user_email' => $data['user_email']]);
            if ($insert) {
                return response()->json(['status' => 'success', 'message' => 'Wishlist Berhasil ditambah.']);  
            }else{
                return response()->json(['status' => 'err', 'message' => 'Terjadi Kesalahan.']);  
            }
        }
    }
    ...
    public function addtocart(Request $request){
        ...
        if(empty(Auth::user()->email)){
            $data['user_email'] = '';    
            return redirect('login-register')->with('flash_message_error','Please Login First!');
        }else{
            $data['user_email'] = Auth::user()->email;
        }
    ...
    public function cart(){           
        ...
        $meta_title = "Shopping Cart - Domi-NATION Website";
        $meta_description = "View Shopping Cart of Domi-NATION Website";
        $meta_keywords = "shopping cart, Domi-NATION Website";
    ...
    public function wishlist(){
        if(Auth::check()){
            $user_email = Auth::user()->email;
            $userWishlist = DB::table('wishlist')
                                ->select('wishlist.*','products.id as productid','products.image','products.product_code','products.product_name','products.price')
        ...
        // }
        /*echo "<pre>"; print_r($userCart); die;*/
        $meta_title = "Wishlist - Domi-NATION Website";
        $meta_description = "View Wishlist of Domi-NATION Website";
        $meta_keywords = "wishlist, Domi-NATION Website";
    ...
    public function checkout(Request $request){
        ...
        $meta_title = "Checkout - Domi-NATION Website";
    ...
    public function orderReview(){
        ...
        $meta_title = "Order Review - Domi-NATION Website";
        ...
        if($request->isMethod('post')){
            ...
            if($data['payment_method']=="COD"){
                ...
                Mail::send('emails.order',$messageData,function($message) use($email){
                    $message->to($email)->subject('Order Placed - Domi-NATION Website');    
                });
                ...
                return redirect('/orders/'.$order_id)->with('flash_message_success','Your Order Success, Please Pay and Upload Bukti Tranfer!');
            }
    ...
    public function uploadBukti(Request $request)
    {
        ...
        if($request->hasFile('filebukti')){
            $image_tmp = Input::file('filebukti');
            if ($image_tmp->isValid()) {
                // Upload Images after Resize
                $extension = $image_tmp->getClientOriginalExtension();
                $fileName = rand(111,99999).'.'.$extension;
                $large_image_path = 'images/backend_images/order/large'.'/'.$fileName;
                $medium_image_path = 'images/backend_images/order/medium'.'/'.$fileName;  
                $small_image_path = 'images/backend_images/order/small'.'/'.$fileName;  
                ...
                return redirect()->back()->with('flash_message_error','File Anda tidak valid.');    
            }
            $update = Order::where('id',$request->get('orderid'))->update(['transfer_attach'=>$bukti_name]);
            return redirect()->back()->with('flash_message_success','Bukti Transfer Berhasil di upload.');    
        }else{
            return redirect()->back()->with('flash_message_error','Harap Upload Bukti Transfer Anda!');
        }
    }
