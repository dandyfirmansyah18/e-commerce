		...
		<div class="container">
				...
				<table class="table table-condensed">
					...
					<tbody>
						...
						<tr>
							...
							<td class="cart_total">
								<p class="cart_total_price">IDR {{ $cart->price*$cart->quantity }}</p>
							</td>
						</tr>
							...
							<td colspan="2">
								<table class="table table-condensed total-result">
									<tr>
										...
										<td>IDR {{ number_format($total_amount,0,',','.') }}</td>
									</tr>
									<tr class="shipping-cost">
										<td>Shipping Cost (+)</td>
										<td>IDR {{ number_format($shippingCharges,0,',','.') }}</td>										
									</tr>
									<tr class="shipping-cost">
										<td>Discount Amount (-)</td>
										<td>
											@if(!empty(Session::get('CouponAmount')))
												IDR {{ number_format(Session::get('CouponAmount'),0,',','.') }}
											@else
												IDR 0
											@endif
										</td>	
									...
									<tr>
										...
										IDR {{ number_format($grand_total,0,',','.') }}</span></td>
									</tr>
								</table>
							</td>
						</tr>
			...
			<form name="paymentForm" id="paymentForm" action="{{ url('/place-order') }}" method="post">{{ csrf_field() }}
				...
				<div class="payment-options">
					...
					<span>
						<label><input type="radio" name="payment_method" id="Transfer" value="Transfer"> <strong>Transfer</strong></label>
					</span>
